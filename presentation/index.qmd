...
- We recommend using RStudio Cloud for ease of use ☁️
  ...
  - However, okay to use other RStudio infrastructure `r fontawesome::fa("server", "white")`
- Clone your repo into RStudio with the `https` link `r fontawesome::fa("clone", "indigo")`
...
```{mermaid}
flowchart TD
  A(Discuss workflow <br> purpose) --> B(Create workflow)
  B --> C(Push workflow)
