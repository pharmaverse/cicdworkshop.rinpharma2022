...
## What could standard CI/CD for R packages look like?
...
## Issue 2: Common CI/CD workflows for admiral upstream and downstream dependencies
...
- Testing and deploying `teal` web applications
- Validation of all packages to adhere to regulatory standards
...
## Solution 1 - Testing packages as a cohort

- For a given package in development, it will be need to be tested against the latest updates that take place in upstream dependencies
- Monorepo emulation via a git branch naming strategy can be achieved by using the [staged.dependencies R package](https://github.com/openpharma/staged.dependencies) and the [staged.dependencies GitHub Action](https://github.com/insightsengineering/staged-dependencies-action).

<div class="centre">
![](https://raw.githubusercontent.com/insightsengineering/hex-stickers/main/thumbs/staged.dependencies.png)
</div>

## Solution 2 - Shiny App Testing & Deployment

- Statistical Programmers and Analysts create Shiny web apps that are created via the `teal` framework for analyzing data
- They are tested continuously via a CI pipeline that uses the [`shinytest2`](https://cran.r-project.org/web/packages/shinytest2/index.html) R package
- They are deployed to an RSConnect or Shiny Server instance via a continuous deployment pipeline
...
<div class="centre">
![](https://rstudio.github.io/shinytest2/articles/images/record-simple-app.png)
</div>

## Solution 3 - Validating R Packages
...
- R packages are validated by an internal validation team that uses CI/CD pipelines to automatically
  ...
  - enforcing certain criteria to make sure that the new package is meeting regulatory quality requirements
...
<div class="centre">
![](images/rbmi-validation-report.png)
</div>
...
## Solution 1 - Reducing the Test Suite Runtime {visibility="uncounted"}
<br/>

<!-- TODO - Potentially can drop this slide, doesn't add value and am short on time -->
...
```
![](images/vscode-tests.png)


::: {.notes}
- Small plug for VScode that you can use it to execute tasks where you fix the environment
variable in order to run the full test suite locally
:::
...
## Solution 4 - Testing Against In-House Servers
<br/>
